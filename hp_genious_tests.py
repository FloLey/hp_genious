...
from hp_genious_request import ask_with_rag,ask_simple,ask_multi_question
...
def answer_question(output_file: str = 'hp_questions_and_answers.csv'):
    results = []

    # Iterate over each question
    for question, correct_answer in tqdm(zip(questions, answers), "question"):
        # Get responses from each function
        simple_answer = ask_simple(question)
        rag_answer = ask_with_rag(question)
        multi_question_answer = ask_multi_question(question)
...
def grade_answers(input_file: str = 'hp_questions_and_answers.csv', output_file: str = 'hp_questions_and_answers_scores.csv'):
    ...
    for index, row in tqdm(df.iterrows(), total=df.shape[0], desc="Grading questions"):
        ...
        for column in answer_columns:
            ...
            total_scores[column] += score  # Accumulate total scores
